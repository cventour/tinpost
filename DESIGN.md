...
- **Direction (`--wire-in` / `--wire-out`):** A transcript line is toned by which way it travelled. What the sender said takes `--wire-in` (`#1b1f24` / `#e7eaee`, 14.8:1 and 14.5:1 on `--sunk`); what Tinpost answered takes `--wire-out` (`#4a5568` / `#aab4c2`, 6.7:1 and 8.3:1) &mdash; one step back, not a whisper, because the reply is the half carrying the refusal anyone is hunting. Both sit *above* `--ink-dim`, which stays the tone for detail lines with no direction at all, giving three readable levels rather than two. Severity still wins: the `.lv-error` and `.lv-warn` rules are declared after the direction rules, at equal specificity, so a `550` reads as a refusal before it reads as outbound.
